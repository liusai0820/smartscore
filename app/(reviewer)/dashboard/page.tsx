...
export default async function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-(--color-ink) pb-24">
      <AutoRefresher
        initialState={stateConfig?.value || 'CLOSED'}
        initialProjectId={currentProjectId}
      />
      {/* 顶部导航栏 */}
      <header className="bg-(--color-ink-light) border-b border-(--color-ink-soft) sticky top-0 z-20">
        <div className="max-w-3xl mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-(--color-text-primary)" style={{ fontFamily: 'var(--font-noto-serif)' }}>
                  智评
                </h1>
            ...
            <form action={LogoutButton}>
              <button className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-(--color-text-muted) hover:text-[#e85a5a] transition-colors rounded-lg hover:bg-[#c53d43]/10">
        ...
        <div className={`px-4 py-2.5 text-center text-sm font-medium flex items-center justify-center gap-2 ${isScoringOpen
          ? 'bg-linear-to-r from-[#7ec699]/10 via-[#7ec699]/20 to-[#7ec699]/10 text-[#7ec699] border-t border-[#7ec699]/20'
          : 'bg-linear-to-r from-[#c53d43]/10 via-[#c53d43]/20 to-[#c53d43]/10 text-[#e85a5a] border-t border-[#c53d43]/20'
          }`}>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="float-card p-5 mb-6">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-(--color-text-secondary) text-sm font-medium">评分进度</span>
            </div>
            <span className="text-[#d4a853] font-bold">{scoredByUser} / {totalProjects}</span>
          </div>
          <div className="relative h-3 bg-(--color-ink-medium) rounded-full overflow-hidden">
            <div
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-(--color-text-primary) flex items-center gap-2 mb-4">
          ...
          {!currentProject ? (
            <div className="float-card p-12 text-center">
              <svg className="w-16 h-16 mx-auto text-(--color-ink-soft) mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-(--color-text-primary) text-lg font-medium mb-2">等待主持人开启项目评分</p>
              <p className="text-(--color-text-muted) text-sm">请耐心等待，当前暂无需要评分的项目</p>
            </div>
          ) : isOwnDepartment ? (
            <div className="float-card p-8 text-center border-2 border-[#d4a853]/30">
              ...
              <p className="text-(--color-text-muted) text-sm">您与该项目属于同一部门，无需评分</p>
              <div className="mt-4 p-4 rounded-lg bg-(--color-ink-medium)">
                <p className="text-(--color-text-primary) font-medium">{currentProject.name}</p>
                <p className="text-(--color-text-muted) text-sm mt-1">{currentProject.department} · {currentProject.presenter}</p>
              </div>
            </div>
          ) : (
